...
function doRectanglesOverlap(rect1, rect2) {
  function Rectangle(initObj) {
    this.x1 = initObj.left;
    this.x2 = initObj.left + initObj.width;
    this.y1 = initObj.top;
    this.y2 = initObj.top + initObj.height;
  }

  const r1 = new Rectangle(rect1);
  const r2 = new Rectangle(rect2);

  return !(r1.y1 > r2.y2 || r1.y2 < r2.y1 || r1.x1 > r2.x2 || r1.x2 < r2.x1);
}
...
function isBracketsBalanced(str) {
  ...
  charArr.forEach((char) => {
    if (openLib.includes(char)) {
      ...
      const openBracket = openLib[closeLib.indexOf(char)];
      if (resultExpr[resultExpr.length - 1] === openBracket) {
        resultExpr.splice(-1, 1);
      } else {
        resultExpr.push(char);
      }
    }
